...
public class TestUtils {
	...
	public static void createProject(ProjectModel projectModel) {
		waitForProcess(5000);
        bot.menu(ProjectParameters.MENU_FILE).menu(ProjectParameters.MENU_NEW).menu(ProjectParameters.MENU_C_CPP_PROJECT).menu(ProjectParameters.MENU_RENESAS_RX).click();
	...
	public static void buildProject(ProjectModel projectModel) {
		waitForProcess(5000);	
		bot.menu(ProjectParameters.MENU_WINDOW).menu(ProjectParameters.MENU_SHOW_VIEW).menu(ProjectParameters.MENU_OTHER).click();
        bot.text().setText(ProjectParameters.WINDOW_PROJECT_EXPLORER);
        SWTBotTreeItem treeItem = bot.tree().getTreeItem("General");
        bot.waitUntil(org.eclipse.swtbot.swt.finder.waits.Conditions.treeItemHasNode(treeItem, ProjectParameters.WINDOW_PROJECT_EXPLORER));
        ...
        waitForProcess(5000);
		bot.tree().getTreeItem(projectModel.getProjectName() +" [HardwareDebug]").contextMenu(ProjectParameters.MENU_BUILD_PROJECT).click();

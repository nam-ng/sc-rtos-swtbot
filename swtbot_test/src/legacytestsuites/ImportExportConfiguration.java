...
public class ImportExportConfiguration {
	...
	public void tc_02_ExportConfig() throws Exception {
		...
		SWTBotTreeItem[] configTree = bot.tree(2).getTreeItem(ProjectParameters.AmazonConfig.CONFIGURATIONS)
				.getItems();
		for (SWTBotTreeItem config : configTree) {
			Utility.changeConfigOfTextBoxWithIndex(config,
					ProjectParameters.AmazonConfig.MAXIMUM_PRIORITIES_APPLICATION_TASK, "8", false);
		}
		...
		configTree = bot.tree(2).getTreeItem(ProjectParameters.AmazonConfig.CONFIGURATIONS).getItems();
		for (SWTBotTreeItem config : configTree) {
			Utility.changeConfigOfTextBoxWithIndex(config, ProjectParameters.AmazonConfig.JSMN_TOKENS, "128", false);
		}
		...
		Utility.reFocus(robot);
		Utility.copyLocationToClipBoard(ProjectParameters.FileLocation.AMAZON_EXPORT);
	...
	public void tc_04_ImportConfig() throws Exception {
		...
		SWTBotTreeItem[] configTree = bot.tree(2).getTreeItem(ProjectParameters.AmazonConfig.CONFIGURATIONS).getItems();
		...
		for (SWTBotTreeItem config : configTree) {
			if (config.cell(0).contains(ProjectParameters.AmazonConfig.JSMN_TOKENS)) {
				isRightValue2 = config.cell(1).equals("64");
			}
		}
		...
		for (SWTBotTreeItem config : configTree) {
			if (config.cell(0).contains(ProjectParameters.AmazonConfig.JSMN_TOKENS)) {
				isRightValue4 = config.cell(1).equals("128");
			}
		}

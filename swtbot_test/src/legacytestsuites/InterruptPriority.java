...
public class InterruptPriority {
	...
	public void tc_02_CheckInterruptPriority() throws Exception {
		bot.perspectiveByLabel("Smart Configurator").activate();
		...
		Utility.reFocus(robot);
		bot.sleep(5000);
		boolean checkConsole = Utility.isConsoleHasString(
				"W04050002: EDMAC0_EINT0 interrupt priority of r_ether_rx should be set lower than configMAX_SYSCALL_INTERRUPT_PRIORITY(4) if ISR calls RTOS API functions.");
		Utility.clickClearConsole();
		...
		Utility.reFocus(robot);
		bot.sleep(5000);
		boolean checkConsole2 = Utility.isConsoleHasString(
				"W04050002: EDMAC0_EINT0 interrupt priority of r_ether_rx should be set lower than configMAX_SYSCALL_INTERRUPT_PRIORITY(4) if ISR calls RTOS API functions.");
	...
	@Test
	public void tc_03_DeleteAmazonProject() throws Exception {
